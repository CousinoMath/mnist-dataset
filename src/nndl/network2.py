...
class Network(object):
    def __init__(self, sizes, cost=CrossEntropyCost):
        ...
        deviates with standard deviation = 1/\\sqrt{\\text{number of
        neurons connecting to the same neuron}}"""
    ...
    def default_weight_initializer(self):
        """Initialize each weight via a Gaussian with mean 0 and
        standard deviation equal to the inverse square root of the
        number of weights connecting to the same neuron. Initialize the
        biases using a standard normal distribution."""
        self.biases = [np.random.randn(y, 1) for y in self.sizes[1:]]
        self.weights = [np.random.randn(y, x) / np.sqrt(x)
                        for x, y in zip(self.sizes[:-1], self.sizes[1:])]
    
    def large_weight_initializer(self):
        """Initialized each bias and weight with a standard normal
        deviate."""
    ...
    def backprop(self, x, y):
        ...
        gradient of the cost function C_x. ``nabla_b`` and ``nabla_w``
        are layer-by-layer lists of numpy arrays similar to
        ``self.biases`` and ``self.weights``, respectively"""
        nabla_b = [np.zeros(b.shape) for b in self.biases]
        nabla_w = [np.zeros(w.shape) for w in self.weights]
        # feed-forward
        activation = x
    ...
    def accuracy(self, data, output_fn=np.argmax):
